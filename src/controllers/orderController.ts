...
export const downloadReport = async (
  ...
  try {
    ...
    const orders: any = await orderService.getAll(
      req.query as any
    );
    ...
    if (format === 'excel') {
      ...
      const addFooter = () => {
        ...
        for (let i = 0; i < pages.count; i++) {
          doc.switchToPage(i);
          doc.fontSize(8);
          doc.text(`Page ${i + 1} of ${pages.count}`, 50, doc.page.height - 50, { align: 'center' });
        }
      };
      ...
      // Table configuration
      const headers = ['S.No', 'Order Number', 'Date', 'Customer', 'Status', 'Payment', 'Total'];
      ...
      orders.data.forEach((order: any, index: number) => {
        if (index % 2 === 0) {
          doc.fillColor('#f9f9f9')
            .rect(startX, currentY - 5, columnWidths.reduce((a, b) => a + b, 0), 20)
            .fill()
            .fillColor('#000000');
        }
        ...
        rowData.forEach((text, i) => {
          ...
          doc.text(text, x, currentY, { width: columnWidths[i], align: i === 0 || i === columnWidths.length - 1 ? 'center' : 'left' });
        });
